...
static void disp_flush(lv_disp_drv_t * disp_drv, const lv_area_t * area, lv_color_t * color_p)
{
    ...
    { 
        LCD_SetCursor(area->x1, y);
        LCD_WR_REG(LCDPara.wramcmd);
        x = area->x2 - area->x1 + 1;
        LCD_DMA_FILL(color_p, x);
        color_p += x;//此处必须加x，不加再次循环时，写入LCD的数据依然是第一次进入的数据
    }    
    /* IMPORTANT!!!
