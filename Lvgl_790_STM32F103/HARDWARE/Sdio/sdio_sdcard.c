...
static u32 CID_Tab[4];
static u32 RCA=0;					//SD卡CSD,CID以及相对地址(RCA)数据
...
SD_Error SD_Init(void)
{
    ...
    __HAL_RCC_GPIOC_CLK_ENABLE();   //使能PORTC,PORTD时钟
    ...
    */
    ...
    {  		    
        ...
        {
            clkdiv = SDIO_TRANSFER_CLK_DIV_USED + 6;	    //V1.1/V2.0卡，设置最高72/12=6Mhz
        }else{
            clkdiv = SDIO_TRANSFER_CLK_DIV_USED; //SDHC等其他卡，设置最高72/6=12Mhz
        } 
        SDIO_Clock_Set(clkdiv); //设置时钟频率,SDIO时钟计算公式:SDIO_CK时钟=SDIOCLK/[clkdiv+2];其中,SDIOCLK固定为72Mhz 
...
//SDIO时钟初始化设置
//clkdiv:时钟分频系数
//CK时钟=SDIOCLK/[clkdiv+2];(SDIOCLK时钟固定为72Mhz)
...
SD_Error SD_PowerON(void)
{
    ...
    /*初始化时的时钟不能大于400KHz*/ 
    SDIO_InitStructure.ClockDiv = SDIO_INIT_CLK_DIV_USED;	/* HCLK = 72MHz, SDIOCLK = 72MHz, SDIO_CK = HCLK/(178 + 2) = 400 KHz */
    SDIO_InitStructure.ClockEdge = SDIO_CLOCK_EDGE_RISING;
    SDIO_InitStructure.ClockBypass = SDIO_CLOCK_BYPASS_DISABLE;  //不使用bypass模式，直接用HCLK进行分频得到SDIO_CK
    SDIO_InitStructure.ClockPowerSave = SDIO_CLOCK_POWER_SAVE_DISABLE;	// 空闲时不关闭时钟电源
    SDIO_InitStructure.BusWide = SDIO_BUS_WIDE_1B;	 				//1位数据线
    SDIO_InitStructure.HardwareFlowControl = SDIO_HARDWARE_FLOW_CONTROL_DISABLE;//硬件流
    SDIO_Init(SDIO, SDIO_InitStructure);

    SDIO_PowerState_ON(SDIO);	//上电状态,开启卡时钟

    __SDIO_ENABLE(SDIO);    //SDIOCK使能 

    for(i=0;i<74;i++)
    {
        SDIO_CmdInitStructure.Argument = 0x0;//发送CMD0进入IDLE STAGE模式命令.
        SDIO_CmdInitStructure.CmdIndex = SD_CMD_GO_IDLE_STATE; //cmd0
        SDIO_CmdInitStructure.Response = SDIO_RESPONSE_NO;  //无响应
        SDIO_CmdInitStructure.WaitForInterrupt = SDIO_WAIT_NO;
        SDIO_CmdInitStructure.CPSM = SDIO_CPSM_ENABLE;  //则CPSM在开始发送命令之前等待数据传输结束。 
    ...
    SDIO_CmdInitStructure.Argument = SD_CHECK_PATTERN;	//发送CMD8,短响应,检查SD卡接口特性
    SDIO_CmdInitStructure.CmdIndex = SDIO_SEND_IF_COND;	//cmd8
    SDIO_CmdInitStructure.Response = SDIO_RESPONSE_SHORT;//r7
    SDIO_CmdInitStructure.WaitForInterrupt = SDIO_WAIT_NO;			 //关闭等待中断
    ...
    SDIO_CmdInitStructure.Argument = 0x00;//发送CMD55,短响应	
    ...
    {																  
        ...
        {	   										   
            ...
            SDIO_CmdInitStructure.Argument = SD_VOLTAGE_WINDOW_SD | SDType;	//发送ACMD41,短响应	
        ...
        while((!validvoltage)&&(count<SD_MAX_VOLT_TRIAL))
        {	   										   				   
            SDIO_CmdInitStructure.Argument = SD_VOLTAGE_WINDOW_MMC;//发送CMD1,短响应	   
...
{
    SDIO_PowerState_OFF(SDIO);//SDIO电源关闭,时钟停止	
	return SD_OK;		  
}   
//初始化所有的卡,并让卡进入就绪状态
//返回值:错误代码
SD_Error SD_InitializeCards(void)
{
    ...
    {
        SDIO_CmdInitStructure.Argument = 0x00;//发送CMD3,短响应 
        SDIO_CmdInitStructure.CmdIndex = SD_CMD_SET_REL_ADDR;	//cmd3
        SDIO_CmdInitStructure.Response = SDIO_RESPONSE_SHORT; //r6
        SDIO_CmdInitStructure.WaitForInterrupt = SDIO_WAIT_NO;
        SDIO_CmdInitStructure.CPSM = SDIO_CPSM_ENABLE;
    ...
    {
        SDIO_CmdInitStructure.Argument = (u32)(rca<<16);//发送CMD3,短响应 
        SDIO_CmdInitStructure.CmdIndex = SD_CMD_SET_REL_ADDR;	//cmd3
        SDIO_CmdInitStructure.Response = SDIO_RESPONSE_SHORT; //r6
        SDIO_CmdInitStructure.WaitForInterrupt = SDIO_WAIT_NO;
        SDIO_CmdInitStructure.CPSM = SDIO_CPSM_ENABLE;
    ...
    {
        RCA = rca;
        SDIO_CmdInitStructure.Argument = (uint32_t)(rca << 16);//发送CMD9+卡RCA,取得CSD,长响应
...
{
    SDIO_CmdInitStructure.Argument =  addr;//发送CMD7,选择卡,短响应	
...
{	  
    ...
    SDIO_DataInitStructure.DataBlockSize= SDIO_DATABLOCK_SIZE_1B; //清除DPSM状态机配置
    SDIO_DataInitStructure.DataLength = 0;
    ...
    if((blksize>0)&&(blksize<=2048)&&((blksize&(blksize-1))==0))
    {
        ...
        SDIO_CmdInitStructure.Argument =  blksize;
    ...
    SDIO_DataInitStructure.DataBlockSize = power<<4;   //清除DPSM状态机配置
    SDIO_DataInitStructure.DataLength = blksize;
    SDIO_DataInitStructure.DataTimeOut = SD_DATATIMEOUT;
    SDIO_DataInitStructure.DPSM = SDIO_DPSM_ENABLE;
    ...
    //从此开始，发命令和读数据不允许间隔超过100us，所以要禁止任务调度，所有关不掉的中断要控制其执行时间
    DISABLE_OS_TASK_SCHEDULER;
    SDIO_CmdInitStructure.Argument =  addr;
    SDIO_CmdInitStructure.CmdIndex = SD_CMD_READ_SINGLE_BLOCK;
    SDIO_CmdInitStructure.Response = SDIO_RESPONSE_SHORT;
    SDIO_CmdInitStructure.WaitForInterrupt = SDIO_WAIT_NO;
    SDIO_CmdInitStructure.CPSM = SDIO_CPSM_ENABLE;
    SDIO_SendCommand(SDIO, &SDIO_CmdInitStructure);         //发送CMD17+从addr地址出读取数据,短响应 
    errorstatus = CmdResp1Error(SD_CMD_READ_SINGLE_BLOCK);  //等待R1响应   
    if(errorstatus!=SD_OK)
    {
        ENABLE_OS_TASK_SCHEDULER;
        return errorstatus;             //响应错误  
    }
 	...
 	if(DeviceMode==SD_POLLING_MODE)						//查询模式,轮询数据	 
    {
        ...
        {
            if(__SDIO_GET_FLAG(SDIO, SDIO_FLAG_RXFIFOHF) != RESET)							//接收区半满,表示至少存了8个字
            {
        ...
        ENABLE_OS_TASK_SCHEDULER;
        if(__SDIO_GET_FLAG(SDIO, SDIO_FLAG_DTIMEOUT) != RESET)		//数据超时错误
        {										   
            __SDIO_CLEAR_FLAG(SDIO, SDIO_FLAG_DTIMEOUT); 	//清错误标志            
            return SD_DATA_TIMEOUT;
        }
        else if(__SDIO_GET_FLAG(SDIO, SDIO_FLAG_DCRCFAIL) != RESET)	//数据块CRC错误
        {
            __SDIO_CLEAR_FLAG(SDIO, SDIO_FLAG_DCRCFAIL);  		//清错误标志
            return SD_DATA_CRC_FAIL;		   
        }
        else if(__SDIO_GET_FLAG(SDIO, SDIO_FLAG_RXOVERR) != RESET) 	//接收fifo上溢错误
        {
            __SDIO_CLEAR_FLAG(SDIO, SDIO_FLAG_RXOVERR);		//清错误标志
            return SD_RX_OVERRUN;		 
        }
        else if(__SDIO_GET_FLAG(SDIO, SDIO_FLAG_STBITERR) != RESET) 	//接收起始位错误
        {
            __SDIO_CLEAR_FLAG(SDIO, SDIO_FLAG_STBITERR);//清错误标志
            return SD_START_BIT_ERR;		 
        }   
        while(__SDIO_GET_FLAG(SDIO, SDIO_FLAG_RXDAVL) != RESET)	//FIFO里面,还存在可用数据
        {
            *tempbuff = SDIO->FIFO;;	//循环读取数据
            tempbuff++;
        }
        __SDIO_CLEAR_FLAG(SDIO, SDIO_STATIC_FLAGS);//清除所有标记
    }
    else //if(DeviceMode==SD_DMA_MODE)
	{
  	    SD_DMA_Config((u32*)buf,blksize,LL_DMA_DIRECTION_PERIPH_TO_MEMORY);
		TransferError = SD_OK;
...
SD_Error SD_ReadMultiBlocks(u8 *buf,long long addr,u16 blksize,u32 nblks)
{
    ...
    SDIO_DataInitStructure.DataBlockSize = 0; //清除DPSM状态机配置
    SDIO_DataInitStructure.DataLength = 0;
    SDIO_DataInitStructure.DataTimeOut = SD_DATATIMEOUT;
    SDIO_DataInitStructure.DPSM = SDIO_DPSM_ENABLE;
    SDIO_DataInitStructure.TransferDir = SDIO_TRANSFER_DIR_TO_CARD;
    ...
    {
        power=convert_from_bytes_to_power_of_two(blksize);	
        SDIO_CmdInitStructure.Argument =  blksize;     //发送CMD16+设置数据长度为blksize,短响应 
        SDIO_CmdInitStructure.CmdIndex = SD_CMD_SET_BLOCKLEN;
        SDIO_CmdInitStructure.Response = SDIO_RESPONSE_SHORT;
        SDIO_CmdInitStructure.WaitForInterrupt = SDIO_WAIT_NO;
        SDIO_CmdInitStructure.CPSM = SDIO_CPSM_ENABLE;
    ...
    SDIO_DataInitStructure.DataBlockSize = power<<4; //nblks*blksize,512块大小,卡到控制器
    SDIO_DataInitStructure.DataLength = nblks*blksize ; 
    SDIO_DataInitStructure.DataTimeOut = SD_DATATIMEOUT ;
    SDIO_DataInitStructure.DPSM = SDIO_DPSM_ENABLE;
    SDIO_DataInitStructure.TransferDir = SDIO_TRANSFER_DIR_TO_SDIO;
    SDIO_DataInitStructure.TransferMode = SDIO_TRANSFER_MODE_BLOCK;
    ...
    //从此开始，发命令和读数据不允许间隔超过100us，所以要禁止任务调度，所有关不掉的中断要控制其执行时间
    DISABLE_OS_TASK_SCHEDULER;
    SDIO_CmdInitStructure.Argument =  addr;//发送CMD18+从addr地址出读取数据,短响应 
    ...
    if(DeviceMode==SD_POLLING_MODE)
    {
        ...
        if(__SDIO_GET_FLAG(SDIO, SDIO_FLAG_DATAEND) != RESET)		//接收结束
        {
            if((SDIO_STD_CAPACITY_SD_CARD_V1_1==CardType)||(SDIO_STD_CAPACITY_SD_CARD_V2_0==CardType)||(SDIO_HIGH_CAPACITY_SD_CARD==CardType))
            {
                SDIO_CmdInitStructure.Argument =  0;//发送CMD12+结束传输
    ...
    {
        SD_DMA_Config((u32*)buf,nblks*blksize,LL_DMA_DIRECTION_PERIPH_TO_MEMORY); 
        TransferError = SD_OK;
        StopCondition = 1;			//多块读,需要发送停止传输指令 
        TransferEnd = 0;				//传输结束标置位，在中断服务置1
        __SDIO_CLEAR_FLAG(SDIO, SDIO_STATIC_FLAGS);
        SDIO->MASK |= (1<<1)|(1<<3)|(1<<8)|(1<<5)|(1<<9);	//配置需要的中断 
        __SDIO_DMA_ENABLE(SDIO); 						//SDIO DMA使能 
        ENABLE_OS_TASK_SCHEDULER;
...
//blksize:块大小	  
//返回值:错误状态
SD_Error SD_WriteBlock(const u8 *buf,long long addr,  u16 blksize)
{
	SD_Error errorstatus = SD_OK;	
	...
	SDIO_CmdInitStructure.Argument = (u32)RCA<<16;//发送CMD13,查询卡的状态,短响应 	
	...
	SDIO_DataInitStructure.DataBlockSize = power<<4; ;	//blksize, 控制器到卡	
	SDIO_DataInitStructure.DataLength = blksize ;
	SDIO_DataInitStructure.DataTimeOut = SD_DATATIMEOUT ;
	SDIO_DataInitStructure.DPSM = SDIO_DPSM_ENABLE;
	SDIO_DataInitStructure.TransferDir = SDIO_TRANSFER_DIR_TO_CARD;
    ...
    //从此开始，发命令和读数据不允许间隔超过100us，所以要禁止任务调度，所有关不掉的中断要控制其执行时间
    DISABLE_OS_TASK_SCHEDULER;    
    SDIO_CmdInitStructure.Argument = addr;//发送CMD24,写单块指令,短响应 	
	...
	if(DeviceMode == SD_POLLING_MODE)       //此种模式写还有问题，向FIFO写第一个字之后就报下溢错误了！！！
	{
		while(!(SDIO->STA&((1<<10)|(1<<4)|(1<<1)|(1<<3)|(1<<9))))//数据块发送成功/下溢/CRC/超时/起始位错误
		{
			if(__SDIO_GET_FLAG(SDIO, SDIO_FLAG_TXFIFOHE) != RESET)							//发送区半空,表示至少存了8个字
			{
        ...
        else if(__SDIO_GET_FLAG(SDIO, SDIO_FLAG_TXUNDERR) != RESET) 	//接收fifo下溢错误
		{
	 		__SDIO_CLEAR_FLAG(SDIO, SDIO_FLAG_TXUNDERR);		//清错误标志
	...
	{
		SD_DMA_Config((u32*)buf,blksize,LL_DMA_DIRECTION_MEMORY_TO_PERIPH);//SDIO DMA配置
   		TransferError = SD_OK;
...
//nblks:要写入的块数
//返回值:错误状态												   
SD_Error SD_WriteMultiBlocks(const u8 *buf,long long addr,u16 blksize,u32 nblks)
{
    SD_Error errorstatus = SD_OK;
    u8  power = 0, cardstate = 0;
    u32 timeout=0,bytestransferred=0;
    ...
    {
        //提高性能
        SDIO_CmdInitStructure.Argument = (u32)RCA<<16;		//发送ACMD55,短响应 	
        SDIO_CmdInitStructure.CmdIndex = SD_CMD_APP_CMD;
        SDIO_CmdInitStructure.Response = SDIO_RESPONSE_SHORT;
        SDIO_CmdInitStructure.WaitForInterrupt = SDIO_WAIT_NO;
        SDIO_CmdInitStructure.CPSM = SDIO_CPSM_ENABLE;
        ...
        SDIO_CmdInitStructure.Argument =nblks;		//发送CMD23,设置块数量,短响应 	 
    ...
    //从此开始，发命令和读数据不允许间隔超过100us，所以要禁止任务调度，所有关不掉的中断要控制其执行时间
    DISABLE_OS_TASK_SCHEDULER;   
    SDIO_CmdInitStructure.Argument =addr;	//发送CMD25,多块写指令,短响应 	  
    ...
    if(DeviceMode==SD_POLLING_MODE)       //此种模式写还有问题，向FIFO写第一个字之后就报下溢错误了！！！
    {
        ...
        if(__SDIO_GET_FLAG(SDIO, SDIO_FLAG_DATAEND) != RESET)		//发送结束
        {															 
            if((SDIO_STD_CAPACITY_SD_CARD_V1_1==CardType)||(SDIO_STD_CAPACITY_SD_CARD_V2_0==CardType)||(SDIO_HIGH_CAPACITY_SD_CARD==CardType))
            {   
                SDIO_CmdInitStructure.Argument =0;         //发送CMD12+结束传输 	  
    ...
    {
        SD_DMA_Config((u32*)buf,nblks*blksize,LL_DMA_DIRECTION_MEMORY_TO_PERIPH);//SDIO DMA配置
        TransferError = SD_OK;
...
SD_Error SD_ProcessIRQSrc(void)
{
	...
	{	 
 	...
 	if(__SDIO_GET_FLAG(SDIO, SDIO_FLAG_DCRCFAIL) != RESET)//数据CRC错误
	{
		__SDIO_CLEAR_FLAG(SDIO, SDIO_FLAG_DCRCFAIL);  		//清错误标志
		SDIO->MASK&=~((1<<1)|(1<<3)|(1<<8)|(1<<14)|(1<<15)|(1<<4)|(1<<5)|(1<<9));//关闭相关中断
 	...
 	if(__SDIO_GET_FLAG(SDIO, SDIO_FLAG_DTIMEOUT) != RESET)//数据超时错误
	{
		__SDIO_CLEAR_FLAG(SDIO, SDIO_FLAG_DTIMEOUT);  			//清中断标志
		SDIO->MASK&=~((1<<1)|(1<<3)|(1<<8)|(1<<14)|(1<<15)|(1<<4)|(1<<5)|(1<<9));//关闭相关中断
	    TransferError = SD_DATA_TIMEOUT;
	    return(SD_DATA_TIMEOUT);
	}
  	if(__SDIO_GET_FLAG(SDIO, SDIO_FLAG_RXOVERR) != RESET)//FIFO上溢错误
	{
		__SDIO_CLEAR_FLAG(SDIO, SDIO_FLAG_RXOVERR);  			//清中断标志
		SDIO->MASK&=~((1<<1)|(1<<3)|(1<<8)|(1<<14)|(1<<15)|(1<<4)|(1<<5)|(1<<9));//关闭相关中断
	    TransferError = SD_RX_OVERRUN;
	    return(SD_RX_OVERRUN);
	}
   	if(__SDIO_GET_FLAG(SDIO, SDIO_FLAG_TXUNDERR) != RESET)//FIFO下溢错误
	{
		__SDIO_CLEAR_FLAG(SDIO, SDIO_FLAG_TXUNDERR);  			//清中断标志
		SDIO->MASK&=~((1<<1)|(1<<3)|(1<<8)|(1<<14)|(1<<15)|(1<<4)|(1<<5)|(1<<9));//关闭相关中断
	    TransferError = SD_TX_UNDERRUN;
	    return(SD_TX_UNDERRUN);
	}
	if(__SDIO_GET_FLAG(SDIO, SDIO_FLAG_STBITERR) != RESET)//起始位错误
	{
		__SDIO_CLEAR_FLAG(SDIO, SDIO_FLAG_STBITERR);  		//清中断标志
		SDIO->MASK&=~((1<<1)|(1<<3)|(1<<8)|(1<<14)|(1<<15)|(1<<4)|(1<<5)|(1<<9));//关闭相关中断
...
SD_Error CmdError(void)
{
	...
	while(timeout--)
	{
	  if(__SDIO_GET_FLAG(SDIO, SDIO_FLAG_CMDSENT) != RESET)break;	//命令已发送(无需响应)	 
	}	    
...
SD_Error CmdResp7Error(void)
{
	...
	{																				    
		...
		__SDIO_CLEAR_FLAG(SDIO, SDIO_FLAG_CTIMEOUT); 			//清除命令响应超时标志
		return errorstatus;
	}	 
	if(status&(1<<6))						//成功接收到响应
	{								   
		errorstatus=SD_OK;
		__SDIO_CLEAR_FLAG(SDIO, SDIO_FLAG_CMDREND); 				//清除响应标志
 	}
	return errorstatus;
}	   
//检查R1响应的错误状态
...
{	  
	...
	if(__SDIO_GET_FLAG(SDIO, SDIO_FLAG_CTIMEOUT) != RESET)					//响应超时
	{																				    
 		__SDIO_CLEAR_FLAG(SDIO, SDIO_FLAG_CTIMEOUT); 				//清除命令响应超时标志
		return SD_CMD_RSP_TIMEOUT;
	}	
 	if(__SDIO_GET_FLAG(SDIO, SDIO_FLAG_CCRCFAIL) != RESET)					//CRC错误
	{																				    
 		__SDIO_CLEAR_FLAG(SDIO, SDIO_FLAG_CCRCFAIL); 				//清除标志
...
SD_Error CmdResp2Error(void)
{
	...
	{								   
		...
		__SDIO_CLEAR_FLAG(SDIO, SDIO_FLAG_CCRCFAIL);		//清除响应标志
 	}
	__SDIO_CLEAR_FLAG(SDIO, SDIO_STATIC_FLAGS);//清除所有标记
 	return errorstatus;								    		 
} 
...
SD_Error SDEnWideBus(u8 enx)
{
	...
	{
		SDIO_CmdInitStructure.Argument = (uint32_t) RCA << 16;//发送CMD55+RCA,短响应	
		SDIO_CmdInitStructure.CmdIndex = SD_CMD_APP_CMD;
		SDIO_CmdInitStructure.Response = SDIO_RESPONSE_SHORT;
		SDIO_CmdInitStructure.WaitForInterrupt = SDIO_WAIT_NO;
		SDIO_CmdInitStructure.CPSM = SDIO_CPSM_ENABLE;
		...
		SDIO_CmdInitStructure.Argument = arg;//发送ACMD6,短响应,参数:10,4位;00,1位.	
...
SD_Error IsCardProgramming(u8 *pstatus)
{
	...
	SDIO_CmdInitStructure.Argument = (uint32_t) RCA << 16; //卡相对地址参数
	SDIO_CmdInitStructure.CmdIndex = SD_CMD_SEND_STATUS;//发送CMD13 	
	SDIO_CmdInitStructure.Response = SDIO_RESPONSE_SHORT;
	SDIO_CmdInitStructure.WaitForInterrupt = SDIO_WAIT_NO;
	SDIO_CmdInitStructure.CPSM = SDIO_CPSM_ENABLE;
	SDIO_SendCommand(SDIO, &SDIO_CmdInitStructure);	
	...
	while(!(status&((1<<0)|(1<<6)|(1<<2))))status=SDIO->STA;//等待操作完成
 	...
 	if(__SDIO_GET_FLAG(SDIO, SDIO_FLAG_CCRCFAIL) != RESET)			//CRC检测失败
	{  
		__SDIO_CLEAR_FLAG(SDIO, SDIO_FLAG_CCRCFAIL);	//清除错误标记
		return SD_CMD_CRC_FAIL;
	}
	if(__SDIO_GET_FLAG(SDIO, SDIO_FLAG_CTIMEOUT) != RESET)			//命令超时 
	{
		__SDIO_CLEAR_FLAG(SDIO, SDIO_FLAG_CTIMEOUT);			//清除错误标记
		return SD_CMD_RSP_TIMEOUT;
	}
...
SD_Error SD_SendStatus(uint32_t *pcardstatus)
{
	...
	SDIO_CmdInitStructure.Argument = (uint32_t) RCA << 16;//发送CMD13,短响应		 
	SDIO_CmdInitStructure.CmdIndex = SD_CMD_SEND_STATUS;
	SDIO_CmdInitStructure.Response = SDIO_RESPONSE_SHORT;
	SDIO_CmdInitStructure.WaitForInterrupt = SDIO_WAIT_NO;
	SDIO_CmdInitStructure.CPSM = SDIO_CPSM_ENABLE;
...
{ 
	...
	SDIO_CmdInitStructure.Argument = (uint32_t)8;	 //发送CMD16,短响应,设置Block Size为8字节	
	...
	SDIO_CmdInitStructure.CmdIndex = SD_CMD_APP_CMD;//发送CMD55,短响应 	
	SDIO_CmdInitStructure.Response = SDIO_RESPONSE_SHORT;
	SDIO_CmdInitStructure.WaitForInterrupt = SDIO_WAIT_NO;
	SDIO_CmdInitStructure.CPSM = SDIO_CPSM_ENABLE;
	SDIO_SendCommand(SDIO, &SDIO_CmdInitStructure);
	...
	SDIO_DataInitStructure.DataTimeOut = SD_DATATIMEOUT;
	SDIO_DataInitStructure.DataLength = 8;  //8个字节长度,block为8字节,SD卡到SDIO.
	SDIO_DataInitStructure.DataBlockSize = SDIO_DATABLOCK_SIZE_8B  ;  //块大小8byte 
	SDIO_DataInitStructure.TransferDir = SDIO_TRANSFER_DIR_TO_SDIO;
	SDIO_DataInitStructure.TransferMode = SDIO_TRANSFER_MODE_BLOCK;
	SDIO_DataInitStructure.DPSM = SDIO_DPSM_ENABLE;
	SDIO_ConfigData(SDIO, &SDIO_DataInitStructure);		
	...
	SDIO_CmdInitStructure.CmdIndex = SD_CMD_SD_APP_SEND_SCR;	//发送ACMD51,短响应,参数为0	
	SDIO_CmdInitStructure.Response = SDIO_RESPONSE_SHORT;  //r1
	SDIO_CmdInitStructure.WaitForInterrupt = SDIO_WAIT_NO;
	SDIO_CmdInitStructure.CPSM = SDIO_CPSM_ENABLE;
	SDIO_SendCommand(SDIO, &SDIO_CmdInitStructure);
	...
	{ 
		if(__SDIO_GET_FLAG(SDIO, SDIO_FLAG_RXDAVL) != RESET)//接收FIFO数据可用
		{
...
{		 
    ...
    LL_DMA_DeInit(DMA2, LL_DMA_CHANNEL_4);              //将DMA2的通道4寄存器重设为缺省值
    LL_DMA_DisableChannel(DMA2, LL_DMA_CHANNEL_4);      //关闭DMA2 通道4

    DMA_InitStruct.PeriphOrM2MSrcAddress  = (u32)&SDIO->FIFO;               //DMA外设基地址
    DMA_InitStruct.MemoryOrM2MDstAddress  = (u32)mbuf;                      //DMA内存基地址
    DMA_InitStruct.Direction              = DMA_DIR;                        //数据传输方向
    DMA_InitStruct.Mode                   = LL_DMA_MODE_NORMAL;             //工作在正常缓存模式
    DMA_InitStruct.PeriphOrM2MSrcIncMode  = LL_DMA_PERIPH_NOINCREMENT;      //外设地址寄存器不变
    DMA_InitStruct.MemoryOrM2MDstIncMode  = LL_DMA_MEMORY_INCREMENT;        //内存地址寄存器递增    
    DMA_InitStruct.PeriphOrM2MSrcDataSize = LL_DMA_PDATAALIGN_WORD;         //数据宽度为32位
    DMA_InitStruct.MemoryOrM2MDstDataSize = LL_DMA_MDATAALIGN_WORD;         //数据宽度为32位
    DMA_InitStruct.NbData                 = bufsize/4;                      //DMA通道的DMA缓存的大小
    DMA_InitStruct.Priority               = LL_DMA_PRIORITY_HIGH;           //DMA通道 x拥有高优先级 
    LL_DMA_Init(DMA2, LL_DMA_CHANNEL_4, &DMA_InitStruct);  
    LL_DMA_EnableChannel(DMA2, LL_DMA_CHANNEL_4);                           //开启DMA2 通道4
}      
