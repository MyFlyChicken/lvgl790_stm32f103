...
typedef struct QueueDefinition
{
	...
	volatile UBaseType_t uxMessagesWaiting;/*< 队列项的数量 */
	UBaseType_t uxLength;			/* 创建队列时指定的数据长度 */
	UBaseType_t uxItemSize;			/*< 队列项单位字节 */

	volatile int8_t cRxLock;		/*< 统计上锁后从队列接收到的队列项数量，没有上锁时为queueUNLOCKED */
	volatile int8_t cTxLock;		/*< 统计上锁后发送到队列的队列项数量，没有上锁时为queueUNLOCK */
